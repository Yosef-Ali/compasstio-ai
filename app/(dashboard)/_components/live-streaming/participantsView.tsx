...
function ParticipantView({ participantId }: ParticipantViewProps) {
  ...
  const isPresenting = mMeeting.localParticipant
  ...
  return (
    <div
      className={`h-full w-full  bg-gray-750 relative overflow-hidden rounded-lg video-cover border-8 border-fuchsia-400`}
    >
      <div
...
export function ParticipantsViewer({ isPresenting }: ParticipantsViewerProps) {
  const theme = useTheme();
  const mMeeting = useMeeting();
  const sideBarMode = false // temporary

  const participantsArray = Array.from(mMeeting?.participants.keys()); // Convert to array
